...
function buildIndex() {
  ...
  // The desktop app lists a session only if a record exists under the account
  // dir it is signed into. A transcript can therefore be present and resumable
  // by the CLI while remaining invisible in the app.
  const byAcctCli = new Map();
  for (const r of registry) {
    if (r.cliSessionId && r.accountUuid) byAcctCli.set(`${r.accountUuid}::${r.cliSessionId}`, r);
  }
  ...
  for (const { key, dir } of CONFIG_DIRS) {
    ...
    for (const t of scanTranscripts(key, dir)) {
      ...
      const ownAccountUuid = (accounts[key] && accounts[key].accountUuid) || null;
      const ownRecord = ownAccountUuid ? byAcctCli.get(`${ownAccountUuid}::${t.cliSessionId}`) || null : null;
      ...
      sessions.push({
        ...
        ownAccountUuid,
        desktopVisible: !!ownRecord,
        desktopRecordFile: ownRecord ? ownRecord.registryFile : null,
      });
    }
  }
